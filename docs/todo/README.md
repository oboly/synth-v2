...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / runtime freshness hygiene | Make sure displayed pages are genuinely fresh and each public market-data writer capability has at most one authorized active host owner, with exactly one only after lifecycle `ACTIVE`. | current active lane — `public_candle_freshness` strict gurkDB preflight passed at `6031a94a…`, but controlled acceptance is blocked before writes because 429 enabled assets map to only 421 current Bitvavo EUR trading markets; candle owner remains `UNASSIGNED`; devlap is disabled and Odroid candle units are masked | Reconcile the eight unavailable enabled symbols in canonical asset metadata, then rerun exact-commit strict preflight, two controlled candle cycles, full-universe persistence checks, and only then production authorization/timer activation. Rotation Pressure remains unchanged. |
