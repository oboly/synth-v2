...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / runtime freshness hygiene | Make sure displayed pages are genuinely fresh and each public market-data writer capability has at most one authorized active host owner, with exactly one only after lifecycle `ACTIVE`. | current active lane — per-capability host-ownership contract corrected (`docs/ops/writer_capability_host_ownership_contract_v1.md`); all four `production_runtime_owner`s UNASSIGNED (rotation-pressure devlap acceptance preserved as SUPERSEDED historical context, not current authorization) pending explicit host selection + preflight + acceptance + containment + authorization; host rollout and repeat Native SHORT acceptance remain open | Run read-only host preflight, select candidate host per capability without authorization, inventory and contain legacy runtimes, authorize exactly one production owner only after acceptance and old-owner containment, deploy Odroid SELECT-only validation, prove zero consumer/reporting/account public writers, then repeat provenance acceptance. Account duplicate retirement remains separate. |
...
approval. The repository writer-provenance contract is implemented; the first
scope-administration boundary adds pure contracts and forward-only constraints
only, with no runtime administration transaction or scope mutation. The
historical 42/42 run rows remain `LEGACY_UNATTRIBUTED`, no attributable
production run has been operationally accepted, and
`WRITER_PROVENANCE_UNATTRIBUTED` remains active. Expansion remains blocked on
separate installed-host provenance acceptance,
single-scope adoption/promotion/removal transactions, writer commit-time
fencing, `NO_CURRENT_MAP` bootstrap semantics, and per-symbol failure isolation.
...
| `native_short_multi_asset_rollout_contract_v1.md` | blocked; administration types/schema defined, runtime transactions absent, provenance acceptance separate | Canonical multi-asset readiness audit, scope-administration ownership, sequential SOL/ETH/XRP review queue, and blocked rollout sequence |
