...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / runtime freshness hygiene | Make sure displayed pages are genuinely fresh and each public market-data writer capability has at most one authorized active host owner, with exactly one only after lifecycle `ACTIVE`. | current active lane — `public_candle_freshness` strict gurkDB preflight passed; eight stale historical-import rows were disabled without history deletion; generic enabled-universe validation now reports 421 enabled assets and zero Bitvavo EUR mismatch; candle owner remains `UNASSIGNED`; devlap is disabled and Odroid candle units are masked | Rerun exact-head strict preflight, then two controlled candle cycles and full-universe persistence/lock checks. Consider production authorization/timer activation only after successful acceptance. Rotation Pressure remains unchanged. |
