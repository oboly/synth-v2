...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / Odroid freshness hygiene | Make sure the displayed pages are genuinely fresh and that disk, timer, or runtime failures cannot silently leave frozen data looking current. | current active lane — first; PR A merged (#106) and live-publishing a valid canonical snapshot on host, formal acceptance record outstanding; P2-B pure freshness evaluator implemented | Record PR A manual host-acceptance, then build PR B as the orchestrator-sequenced single Profit Plan writer. Retirement of both legacy user-level families remains a separate host rollout with per-family rollback. |
