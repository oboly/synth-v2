...
| **C** | P2 | Short Swing / Odroid freshness hygiene | Make sure the displayed pages are genuinely fresh and that disk, timer, or runtime failures cannot silently leave frozen data looking current. | open operational lane — second | Ownership proven (2026-07-15 audit): the safe system-timer runs in parallel with two legacy user-level families (`synth-account-wallet-dashboard@` — Profit Plan without deltas — and `synth-account-wallet-refresh@`). Forward plan recorded as PR A (persisted native SHORT snapshot) + PR B (single-writer Profit Plan runner invoked as an orchestrator stage); retirement of both legacy families is a separate rollout with per-family rollback. |
