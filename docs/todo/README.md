...
| `profit_plan_live_ladder.md` | **A — active P1 / later** — migrated (corrected): status/priority now owned by Issues #267, #268, #269, #273 (new), #254, #202, #203, #206 and #201 (existing) | Safe prerequisites and ordered path from canonical ladder truth to authenticated preview, decision gate, execution plan, executor, and a tightly controlled live canary |
| `manual_execution_ladder_future_readiness_backlog_v1.md` | **A — P0 items 1-6 remediation reviewed BLOCK/REJECT (2026-07-25); item 7 (F12) request contract + canonical service entrypoint implemented (2026-07-26), reservation/reconciliation/live wiring still open** — migrated: status/priority now owned by Issues #202, #203, #206, and P2 item 13 by Issue #319 (Batch 6E) | Prioritized remediation backlog from the manual execution ladder future-readiness audit (`docs/architecture/manual_execution_ladder_future_readiness_audit_v1.md`); feeds into lane A, does not reorder it |
| `decision_gate_account_protections_v1.md` | **future design P2** — migrated: P1 contract-design status/priority owned by Issue #227; P2 runtime implementation now owned by Issue #318 (Batch 6E) | Account-aware drawdown, loss, stoploss-streak, and cooldown permission blocks inside `decision_gate`; no market ranking or execution authority |
...
| `market_intelligence/sector_rotation_engine_v1.md` | **D — Phase B accepted; migration applied and persisted cohort accepted** | Participation, relative-strength, persistence, and proxy-rotation analytics |
...
| `native_short_multi_asset_rollout_contract_v1.md` | in progress; SOL promoted and accepted, ETH/XRP approved (not yet promoted), 16-scope bounded batch also approved (PR #316), bootstrap manifest generalized to a reviewed list, per-scope failure isolation implemented (#200 closed) — migrated: status/priority now owned by Issues #198, #199, #200 (closed), #276 (Batch 6E re-verified: no residual uncovered scope) | Canonical multi-asset readiness audit, scope-administration ownership, sequential rollout, bootstrap/isolation architecture now implemented |
| `position_rotation_preview.md` | MVP implemented / parked follow-up — migrated: "Next Strategy Work" research follow-up status/priority owned by Issue #230; the implemented MVP cockpit is historical record only, confirmed no residual scope (Batch 6E) | Account-aware read-only cockpit/rotation preview; no active v2.23 priority |
...
| `regime_research.md` | parked — migrated: Phase 1 (P1 reruns/reads, P1 discovered-regime pass) status/priority owned by Issue #231; Phase 2 (breath profile, lead-lag replay, interaction audit) now owned by Issue #322 (Batch 6E); Phase 3 remains explicitly gated behind Phase 2, no Issue required yet | Rotation replay, discovered regimes, symbol profiles, and interaction audits |
...
| `ui_webview.md` | open / secondary — migrated: the 4 accepted Profit Plan coin-card scanability items status/priority owned by Issue #233; UI/chart framework stabilization now owned by Issue #325 (Batch 6E); "Later UI v2 direction" remains historical roadmap prose, no Issue required | Non-blocking UI and styling work after correctness lanes |
...
| `strategy_candidates.md` | parked / open research — migrated: "P1 — Current strategy audit follow-up" status/priority owned by Issue #232; dual-bucket/super-bull backtest research now owned by Issue #323, 168h swing-pullback lead by Issue #324, horizon-bucket design review by Issue #243 (Batch 6E); regime-classifier build remains gated behind #322; `MACRO_DIP_BUDGET_MODE_V1` and legacy Synth v1 review remain parked/no Issue (no current evidence of demand) | Strategy audit and later classifier/policy research |
...
| `docs/archive/paper_candidate_contract.md` | **ARCHIVED (Batch 6F2)** — superseded: canonical proposal contract now `docs/architecture/strategy_proposal_contract_v1.md` (PR #257) | Historical record only; safe adapter design from validated research candidates to `decision_gate` |
| `docs/archive/parked_backlog.md` | **ARCHIVED (Batch 6F2)** — all sections deliberately parked, no Issue required | Historical record only; archived A+ and external-narrative follow-ups |
| `watchlist_candidates.md` | open intake — migrated: the remaining KITE liquidity/spread/candle-history/min-order check owned by Issue #234; other/future watchlist candidates are not an umbrella backlog and each gets its own Issue when it arises (Batch 6E, precedent Issue #131), no residual scope | Human-thesis watchlist intake before validation |
