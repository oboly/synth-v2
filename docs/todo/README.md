...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / Odroid freshness hygiene | Make sure the displayed pages are genuinely fresh and that disk, timer, or runtime failures cannot silently leave frozen data looking current. | current active lane — first; PR A merged (#106) with formal host acceptance recorded 2026-07-17 (single-cycle dry-run + isolated temp publish PASS, canonical untouched); P2-B pure freshness evaluator implemented; multi-cycle acceptance (P2-C) still open | PR B (#113) unblocked for review — build it as the orchestrator-sequenced single Profit Plan writer. Retirement of both legacy user-level families remains a separate host rollout with per-family rollback. |
