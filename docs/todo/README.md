...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / runtime freshness hygiene | Make sure displayed pages are genuinely fresh and each public market-data writer capability has at most one authorized active host owner, with exactly one only after lifecycle `ACTIVE`. | current active lane — `public_candle_freshness` strict gurkDB preflight and two controlled cycles passed after eight stale historical-import rows were disabled without history deletion; 421/421 persisted coverage and zero Bitvavo EUR mismatch; candle owner remains `UNASSIGNED`; devlap is disabled and Odroid candle units are masked | Rerun exact-head strict preflight on the acceptance record, then perform separately authorized production cutover only with administrator-capable sudo. Rotation Pressure remains unchanged. |
