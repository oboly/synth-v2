...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / runtime freshness hygiene | Make sure displayed pages are genuinely fresh and public market-data writers have one host owner. | current active lane — devlap sole-writer repository contract in implementation; host rollout and repeat Native SHORT acceptance remain open | Deploy/accept devlap public-price and candle owners, deploy Odroid SELECT-only validation, prove zero Odroid public writers, then repeat provenance acceptance. Account duplicate retirement remains separate. |
