...
|---:|---:|---|---|---|---|
| **C** | P2 | Short Swing / runtime freshness hygiene | Make sure displayed pages are genuinely fresh and each public market-data writer capability has exactly one explicitly selected host owner. | current active lane — per-capability host-ownership contract corrected (`docs/ops/writer_capability_host_ownership_contract_v1.md`); all four `production_runtime_owner`s UNASSIGNED (rotation-pressure devlap acceptance preserved as SUPERSEDED historical context, not current authorization) pending explicit host selection + preflight + acceptance; host rollout and repeat Native SHORT acceptance remain open | Run read-only host preflight, select production owner per capability with separate evidence, deploy Odroid SELECT-only validation, prove zero consumer/reporting/account public writers, then repeat provenance acceptance. Account duplicate retirement remains separate. |
