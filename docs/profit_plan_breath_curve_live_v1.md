...
- `anchor_ts_utc`: resolved cycle anchor behind the live checkpoint when available
- `anchor_search_days`: deterministic rolling anchor search window
...
- `resolver_name`: auditable live resolver identifier
- `resolver_version`: shared core/resolver version
- `resolver_candidate_count`: ranked anchor/offset candidates considered before selection
- `resolver_rank_basis`: exact deterministic tie-break order used by the resolver
...
- The shared matcher/progression/band logic lives in `src/market_context/breath_curve_core_v1.py`; runtime does not import any `src.research.run_*` or backtest module.
...
- Historical replay validation against persisted Breath Curve partial-to-full artifact rows stays read-only and off the request path.
