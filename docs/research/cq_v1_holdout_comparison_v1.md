...
The runner consumes only immutable research artifacts:
...
Only forward-label rows with `status=COMPLETE` enter metric calculations. Incomplete horizon coverage remains excluded evidence, never a zero return.
...
The output directory must be empty/new. Failed runs emit `summary.json` with `terminal_state=FAILED`.
