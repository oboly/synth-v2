...
Both use `as_of_ts_utc` and `model_version`. The aggregate snapshot exposes breadth and market-direction context, while the per-asset observation exposes asset-level rotation pressure.
