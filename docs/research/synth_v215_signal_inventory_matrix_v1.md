...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `obs_market_candle` | `obs_market_candle` / `src.etl.bitvavo.run_candles_etl` | devlap `scripts/run_market_candle_freshness_once.sh`; the 4h chain validates persisted candles SELECT-only | market observation | MIXED | `15m/1h/4h/1d/1w` | NO | YES | per interval freshness contract | ACTIVE | Root public market observation. Odroid is a persisted-state consumer and has no candle writer. Not itself a directional signal. |
...
| `sparse_candle_gap_diagnostics` | `asset` + exact-key `obs_market_candle` -> `asset_interval_quality` | `scripts/run_chain_4h.sh` | quality diagnostics | MIXED | `1h/4h/1d` | NO | YES | interval freshness and gap audit | ACTIVE | Gap events, missing candles, and coverage ratio are active diagnostics, not directional signals. |
