...
baseline ranks within the same split. No fit crosses split boundaries.
...
The harness reports all three frozen split labels but does not tune anything.
Any future threshold or feature-selection rule must be chosen using discovery
only, frozen, and then evaluated unchanged on validation/holdout.
