# Native SHORT ALGO bootstrap-promotion approval
...
for the exact ALGO scope below (the two blockers this manifest still narrows
...
The operator explicitly approved ALGO as one of the next native SHORT
...
The operator identified ALGO as one of 16 readiness-qualified, previously
...
against ALGO; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against ALGO immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact ALGO scope,
  ...
  `fa3e2dc6d386ffbeae51cfa88e8250d0fc9f3fc17718cf64dc9d670d541aa360`.
...
- exactly one `PROMOTE_SCOPE` invocation for the ALGO scope above, executed
  ...
  XRP, at ALGO's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of ALGO after the first succeeds (structurally
  impossible via this same bootstrap path once ALGO's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for ALGO or any other scope (`REMOVAL_CONTRACT_MISSING`
