# Native SHORT HBAR bootstrap-promotion approval
...
for the exact HBAR scope below (the two blockers this manifest still narrows
...
The operator explicitly approved HBAR as one of the next native SHORT
...
The operator identified HBAR as one of 16 readiness-qualified, previously
...
against HBAR; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against HBAR immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact HBAR scope,
  ...
  `1c5997be7efb20885f19cf82212d04c89e30bcdf80524cdaaa69c34b264b1241`.
...
- exactly one `PROMOTE_SCOPE` invocation for the HBAR scope above, executed
  ...
  XRP, at HBAR's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of HBAR after the first succeeds (structurally
  impossible via this same bootstrap path once HBAR's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for HBAR or any other scope (`REMOVAL_CONTRACT_MISSING`
