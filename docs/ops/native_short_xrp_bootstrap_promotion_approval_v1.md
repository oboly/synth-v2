...
# Native SHORT XRP bootstrap-promotion approval
...
(`docs/ops/native_short_sol_bootstrap_promotion_approval_v1.md`) and ETH
...
for the exact XRP scope below (the two blockers this manifest still narrows
...
The operator explicitly approved XRP as the next native SHORT managed
production canary after SOL and ETH, for exactly this scope and no other:
...
This approval is limited to this exact scope. It does not approve ETH, SUI,
BTC (BTC is a separate, already-legacy scope handled by
`ADOPT_LEGACY_SCOPE`, not this bootstrap path), or any broader rollout. ETH
is approved separately and independently in
`docs/ops/native_short_eth_bootstrap_promotion_approval_v1.md`; neither
...
artifact) classified XRP `READY_FOR_SEQUENTIAL_CANARY_REVIEW`: market-ready
...
-- a genuine first-ever administration attempt). XRP ranked #2 of 17
...
non-self-referential model as the existing SOL and ETH approvals).
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact XRP scope,
...
- exactly one `PROMOTE_SCOPE` invocation for the XRP scope above, executed
  ...
  and ETH in the documented sequential rollout order.

Not approved by this record:

- ETH, SUI, or any other symbol;
- a second promotion of XRP after the first succeeds (structurally
  impossible via this same bootstrap path once XRP's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for XRP or any other scope (`REMOVAL_CONTRACT_MISSING`
