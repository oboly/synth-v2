# Native SHORT FLOKI bootstrap-promotion approval
...
for the exact FLOKI scope below (the two blockers this manifest still narrows
...
The operator explicitly approved FLOKI as one of the next native SHORT
...
The operator identified FLOKI as one of 16 readiness-qualified, previously
...
against FLOKI; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against FLOKI immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact FLOKI scope,
  ...
  `e3f2ce2077d67a842aa1470e5a4712bbbac589a2179a89e8dca4dbc7ee75ad2e`.
...
- exactly one `PROMOTE_SCOPE` invocation for the FLOKI scope above, executed
  ...
  XRP, at FLOKI's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of FLOKI after the first succeeds (structurally
  impossible via this same bootstrap path once FLOKI's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for FLOKI or any other scope (`REMOVAL_CONTRACT_MISSING`
