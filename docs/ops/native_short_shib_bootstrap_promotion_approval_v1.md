# Native SHORT SHIB bootstrap-promotion approval
...
for the exact SHIB scope below (the two blockers this manifest still narrows
...
The operator explicitly approved SHIB as one of the next native SHORT
...
The operator identified SHIB as one of 16 readiness-qualified, previously
...
against SHIB; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against SHIB immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact SHIB scope,
  ...
  `065cdc5684bf2aba53eeb9d91b6d5ba87788aa037661a889e93f0058ac38df7e`.
...
- exactly one `PROMOTE_SCOPE` invocation for the SHIB scope above, executed
  ...
  XRP, at SHIB's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of SHIB after the first succeeds (structurally
  impossible via this same bootstrap path once SHIB's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for SHIB or any other scope (`REMOVAL_CONTRACT_MISSING`
