# Native SHORT ICP bootstrap-promotion approval
...
for the exact ICP scope below (the two blockers this manifest still narrows
...
The operator explicitly approved ICP as one of the next native SHORT
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact ICP scope,
  ...
  `6bbc1023c2ea535f7cc7ff7fb6893aabb199a87804b4d1281a030aedde04c9ac`.
...
- exactly one `PROMOTE_SCOPE` invocation for the ICP scope above, executed
  ...
  XRP, at ICP's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of ICP after the first succeeds (structurally
  impossible via this same bootstrap path once ICP's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for ICP or any other scope (`REMOVAL_CONTRACT_MISSING`
