# Native SHORT MOG bootstrap-promotion approval
...
for the exact MOG scope below (the two blockers this manifest still narrows
...
The operator explicitly approved MOG as one of the next native SHORT
...
The operator identified MOG as one of 16 readiness-qualified, previously
...
against MOG; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against MOG immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact MOG scope,
  ...
  `feeaf6a30a2a29e50f42ef71f1b4f1d90176f16891449c19e38e20b0b1ebf2ae`.
...
- exactly one `PROMOTE_SCOPE` invocation for the MOG scope above, executed
  ...
  XRP, at MOG's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of MOG after the first succeeds (structurally
  impossible via this same bootstrap path once MOG's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for MOG or any other scope (`REMOVAL_CONTRACT_MISSING`
