# Native SHORT BNB bootstrap-promotion approval
...
for the exact BNB scope below (the two blockers this manifest still narrows
...
The operator explicitly approved BNB as one of the next native SHORT
...
The operator identified BNB as one of 16 readiness-qualified, previously
...
against BNB; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against BNB immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact BNB scope,
  ...
  `35f23a27d6f261345e52bc159a17e922251ad2d149a1082613db0082cf46a957`.
...
- exactly one `PROMOTE_SCOPE` invocation for the BNB scope above, executed
  ...
  XRP, at BNB's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of BNB after the first succeeds (structurally
  impossible via this same bootstrap path once BNB's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for BNB or any other scope (`REMOVAL_CONTRACT_MISSING`
