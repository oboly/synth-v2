# Native SHORT XPL bootstrap-promotion approval
...
for the exact XPL scope below (the two blockers this manifest still narrows
...
The operator explicitly approved XPL as one of the next native SHORT
...
The operator identified XPL as one of 16 readiness-qualified, previously
...
against XPL; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against XPL immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact XPL scope,
  ...
  `86e256107b4dd0ca1c7f5e969048b13341a3e0e53cc9aca825d309a83629d65a`.
...
- exactly one `PROMOTE_SCOPE` invocation for the XPL scope above, executed
  ...
  XRP, at XPL's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of XPL after the first succeeds (structurally
  impossible via this same bootstrap path once XPL's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for XPL or any other scope (`REMOVAL_CONTRACT_MISSING`
