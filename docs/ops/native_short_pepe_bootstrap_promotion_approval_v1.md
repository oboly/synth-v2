# Native SHORT PEPE bootstrap-promotion approval
...
for the exact PEPE scope below (the two blockers this manifest still narrows
...
The operator explicitly approved PEPE as one of the next native SHORT
...
The operator identified PEPE as one of 16 readiness-qualified, previously
...
against PEPE; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against PEPE immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact PEPE scope,
  ...
  `79c5ae729df550d67e5dba45d827cda54cf250dbeb41755d57679b2d3f0c64f3`.
...
- exactly one `PROMOTE_SCOPE` invocation for the PEPE scope above, executed
  ...
  XRP, at PEPE's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of PEPE after the first succeeds (structurally
  impossible via this same bootstrap path once PEPE's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for PEPE or any other scope (`REMOVAL_CONTRACT_MISSING`
