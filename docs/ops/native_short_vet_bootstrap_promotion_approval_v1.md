# Native SHORT VET bootstrap-promotion approval
...
for the exact VET scope below (the two blockers this manifest still narrows
...
The operator explicitly approved VET as one of the next native SHORT
...
The operator identified VET as one of 16 readiness-qualified, previously
...
against VET; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against VET immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact VET scope,
  ...
  `bcf1c122ea612a032b259e437644a29f4104d310282bfc387ec6a9ec5f8a95f0`.
...
- exactly one `PROMOTE_SCOPE` invocation for the VET scope above, executed
  ...
  XRP, at VET's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of VET after the first succeeds (structurally
  impossible via this same bootstrap path once VET's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for VET or any other scope (`REMOVAL_CONTRACT_MISSING`
