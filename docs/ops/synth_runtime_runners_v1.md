...
- **Legacy/manual path:** `scripts/odroid/run_linked_profile_dashboard_refresh_once.sh`
  ...
  scheduled/runtime caller is added. The manual/acceptance path also shares the canonical `SYNTH_LINKED_PROFILE_RUNTIME_LOCK` / `/tmp/synth-linked-profile-runtime-orchestrator.lock` lock domain and skips when the scheduled owner is active, preventing concurrent legacy writes.
