# Native SHORT SUI bootstrap-promotion approval
...
for the exact SUI scope below (the two blockers this manifest still narrows
...
The operator explicitly approved SUI as one of the next native SHORT
...
The operator identified SUI as one of 16 readiness-qualified, previously
...
against SUI; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against SUI immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact SUI scope,
  ...
  `076781ca77e23ebc7d25072e64a364f3fbd4341d00ea64f045e020451c2b4cca`.
...
- exactly one `PROMOTE_SCOPE` invocation for the SUI scope above, executed
  ...
  XRP, at SUI's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of SUI after the first succeeds (structurally
  impossible via this same bootstrap path once SUI's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for SUI or any other scope (`REMOVAL_CONTRACT_MISSING`
