# Native SHORT HOT bootstrap-promotion approval
...
for the exact HOT scope below (the two blockers this manifest still narrows
...
The operator explicitly approved HOT as one of the next native SHORT
...
The operator identified HOT as one of 16 readiness-qualified, previously
...
against HOT; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against HOT immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact HOT scope,
  ...
  `5396257d63fc9e78d4b7e6b55f8589ce9b5e84a71c310900631f47621760685f`.
...
- exactly one `PROMOTE_SCOPE` invocation for the HOT scope above, executed
  ...
  XRP, at HOT's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of HOT after the first succeeds (structurally
  impossible via this same bootstrap path once HOT's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for HOT or any other scope (`REMOVAL_CONTRACT_MISSING`
