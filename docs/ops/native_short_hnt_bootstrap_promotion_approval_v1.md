# Native SHORT HNT bootstrap-promotion approval
...
for the exact HNT scope below (the two blockers this manifest still narrows
...
The operator explicitly approved HNT as one of the next native SHORT
...
The operator identified HNT as one of 16 readiness-qualified, previously
...
against HNT; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against HNT immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact HNT scope,
  ...
  `2d8de2bd145c8b8d8c4cd2c96b20d4eba2a06a67141a5f00d9aef45e55a25290`.
...
- exactly one `PROMOTE_SCOPE` invocation for the HNT scope above, executed
  ...
  XRP, at HNT's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of HNT after the first succeeds (structurally
  impossible via this same bootstrap path once HNT's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for HNT or any other scope (`REMOVAL_CONTRACT_MISSING`
