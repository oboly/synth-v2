# Native SHORT AAVE bootstrap-promotion approval
...
for the exact AAVE scope below (the two blockers this manifest still narrows
...
The operator explicitly approved AAVE as one of the next native SHORT
...
The operator identified AAVE as one of 16 readiness-qualified, previously
...
against AAVE; the eligibility determination and its "readiness-qualified"
...
`native_short_multi_asset_audit_v1` run against AAVE immediately before any
...
- **approval-evidence digest** -- this entry's own `approval_evidence_digest`
  is a deterministic SHA-256 over `accepted`, the exact AAVE scope,
  ...
  `6b18cdff3b6942bd518e9876fd92ddadeb3a72e75325252de00e4e5dbd8921ba`.
...
- exactly one `PROMOTE_SCOPE` invocation for the AAVE scope above, executed
  ...
  XRP, at AAVE's own position in the documented sequential rollout order.

Not approved by this record:

- SOL, ETH, XRP, BTC, or any of the other 15 symbols in this batch;
- a second promotion of AAVE after the first succeeds (structurally
  impossible via this same bootstrap path once AAVE's scope row exists --
  see `native_short_promotion_bootstrap_evidence_v1.py`);
- `REMOVE_SCOPE` for AAVE or any other scope (`REMOVAL_CONTRACT_MISSING`
