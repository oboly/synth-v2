...
def build_profit_plan_input_audit_rows(
    *,
    ...
    for market in markets:
        ...
        has_existing_open_orders = open_order_count > 0
        ...
        if not has_existing_open_orders:
...
def format_summary(rows: list[ProfitPlanInputAuditRow], *, broker_mode: str) -> str:
    ...
    for row in rows:
        ...
        lines.append(
            f"{row.symbol}: would_render_state={row.would_render_state} "
...
def main() -> int:
    ...
    if args.live_broker:
        try:
            ...
            orders = [normalize_broker_order(row) for row in raw_orders]
            balances = [normalize_broker_balance(row) for row in raw_balances]
