...
def fetch_bounded_selection_candidates(
    ...
    *,
    ...
    # The LIMIT lives inside a derived table so MariaDB must bound the asset
    # population before resolving historical latest-row lookups. Each lookup is
    # an indexed probe on (asset_id, venue, interval_code, timestamp).
    sql = f"""
    SELECT
        a.asset_id,
        a.symbol,
    ...
    return [
        SelectionCandidate(
            ...
            latest_quality_asof_ts_utc=(
                None if row.get("latest_quality_asof_ts_utc") is None else str(row["latest_quality_asof_ts_utc"])
            ),
...
def fetch_bounded_evidence_timestamps(
    ...
    *,
    ...
    return {
        int(row["asset_id"]): {key: (None if row.get(key) is None else str(row[key])) for key in keys}
        for row in rows
    }
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"PHASE_END name=bounded_fetch candidates={len(candidates)} rows={len(selection_rows)} "
            f"evidence_sets={len(evidence)} elapsed_s={time.perf_counter()-phase:.3f}",
            flush=True,
        )
        ...
        written = write_shadow_rows(conn, rows) if args.write_db else 0
        print(
