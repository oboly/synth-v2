...
REPORT_NAME = "aplus_table1_table2_outcome_validation_v1"
PARSER_VERSION = "0.1"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate A+ Table 1/Table 2 labels against forward market outcomes (single-snapshot, research-only)."
    )
    parser.add_argument(
        "--joined-path",
        default="data/research/aplus_table1_table2_normalized_v1/table1_table2_joined_20260515_1244.jsonl",
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="data/research/aplus_table1_table2_outcome_validation_v1",
    )
...
def compute_outcome_row(
    ...
    base: dict[str, Any] = {
        "prediction_ts_utc": joined_row["prediction_ts_utc"],
        "token": token,
        "asset_id": asset_id,
    ...
    if asset_id is None:
        base["validation_status"] = "NO_ASSET"
        return base

    base_candle = fetch_base_candle(conn, asset_id, venue, interval, prediction_ts)
    ...
    if base_price is None or base_price <= 0:
        base["validation_status"] = "NO_BASE_PRICE"
        return base
    ...
    if future_price is None:
        base["validation_status"] = "NO_FUTURE_PRICE"
        return base
...
def build_aggregations(outcome_rows: list[dict[str, Any]], horizons: list[int]) -> dict[str, Any]:
    ...
    valid_rows = [r for r in outcome_rows if r["validation_status"] == "VALID"]
    ...
    for horizon in horizons:
        ...
        for field, by_value in per_horizon.get("single_field", {}).items():
            for value, metrics in by_value.items():
                if metrics["n_with_return"] is None or metrics["n_with_return"] < 2:
                    continue
                candidates.append({
        ...
        candidates.sort(key=lambda r: (r["avg_return_pct"] if r["avg_return_pct"] is not None else 0.0), reverse=True)
...
def horizon_coverage_summary(outcome_rows: list[dict[str, Any]], horizons: list[int]) -> dict[str, Any]:
    coverage: dict[str, Any] = {}
    for horizon in horizons:
        horizon_rows = [r for r in outcome_rows if r["horizon_hours"] == horizon]
        status_counts: dict[str, int] = {}
        for r in horizon_rows:
            status_counts[r["validation_status"]] = status_counts.get(r["validation_status"], 0) + 1
        valid = sum(1 for r in horizon_rows if r["validation_status"] == "VALID")
        coverage[f"horizon_{horizon}h"] = {
            "total": len(horizon_rows),
            "valid": valid,
            "status_counts": status_counts,
            "missing_tokens": sorted([r["token"] for r in horizon_rows if r["validation_status"] != "VALID"]),
        }
    return coverage
...
def render_table_summary(summary: dict[str, Any]) -> str:
    ...
    lines.append(f"prediction_ts_utc={summary['prediction_ts_utc']}")
    ...
    for horizon, cov in summary["coverage"].items():
        lines.append(f"{horizon}: total={cov['total']} valid={cov['valid']} status={cov['status_counts']}")

    lines.append("")
    lines.append("--- per-horizon overall ---")
    for horizon in summary["horizons"]:
        ...
        lines.append(
            f"horizon_{horizon}h: n={n} avg_return={_fmt(avg)} win_rate={_fmt(wr)} avg_mfe={_fmt(mfe)} avg_mae={_fmt(mae)}"
        )
    ...
    for horizon in summary["horizons"]:
        ...
        for g in top:
            lines.append(
                f"  {g['group_type']}:{g['field']}={g['value']} n={g['n_with_return']} avg_return={_fmt(g['avg_return_pct'])} win_rate={_fmt(g['win_rate_pct'])}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    joined_rows = load_joined(joined_path)
    if not joined_rows:
        print("ERROR: joined file is empty")
        return 2

    prediction_ts_utc = str(joined_rows[0]["prediction_ts_utc"])
    prediction_ts = parse_ts(prediction_ts_utc)

    tokens = sorted({str(r["token"]).upper() for r in joined_rows})

    conn = get_connection()
    try:
        asset_map = fetch_asset_map(conn, tokens)
        ...
        for row in joined_rows:
            ...
            for horizon in horizons:
                outcome_rows.append(
                    compute_outcome_row(
                        conn, row, asset_id, str(args.venue), str(args.interval),
                        prediction_ts, horizon,
                    )
                )
    finally:
        conn.close()

    coverage = horizon_coverage_summary(outcome_rows, horizons)
    ...
    output_paths = {
        "label_outcomes_jsonl": str(out_dir / f"label_outcomes_{slug}.jsonl"),
        "validation_summary_json": str(out_dir / f"validation_summary_{slug}.json"),
    }

    tokens_missing_asset = sorted([t for t in tokens if t not in asset_map])
