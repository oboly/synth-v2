...
def _validate_window(start: datetime, end: datetime) -> None:
    if not isinstance(start, datetime) or not isinstance(end, datetime):
        raise ValueError("effective authority timestamps must be datetimes")
    try:
        duration = end - start
    except TypeError as exc:
        raise ValueError("effective authority timestamps must use compatible UTC form") from exc
...
def _row_to_grant(row: Any) -> ExecutionLiveAuthorityGrantV1:
    try:
        ...
        _validate_window(grant.effective_from_ts_utc, grant.effective_until_ts_utc)
...
class ExecutionLiveAuthorityRepositoryV1:
    ...
    def grant(
        ...
        *,
        ...
        _validate_window(effective_from_ts_utc, effective_until_ts_utc)
    ...
    def revoke(
        ...
        *,
        ...
        revoked_ts_utc = revoked_ts_utc or trusted_clock.utc_now()
        if not isinstance(revoked_ts_utc, datetime):
            raise ValueError("revoked_ts_utc must be a datetime")
    ...
    def resolve_effective(
        ...
        *,
        ...
        if not isinstance(as_of_ts_utc, datetime):
            raise ValueError("as_of_ts_utc must be a datetime")
...
def require_execution_live_authority_v1(
    *,
    ...
    as_of_ts_utc = as_of_ts_utc or trusted_clock.utc_now()
    try:
