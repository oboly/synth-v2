...
DEFAULT_OUTPUT_DIR = Path("artifacts/backtest")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Policy backtest over selection_state snapshots with simple fixed-horizon exits."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Round-trip fees in basis points subtracted from each trade return. Example: 20 = 0.20%% total.",
    )
    parser.add_argument(
        "--max-open-trades-per-asset",
        dest="max_open_trades_per_asset",
        type=int,
        default=1,
        help="Max overlapping trades per asset for a policy. Use 1 for non-overlap per asset.",
    )
...
def apply_non_overlap_per_asset(trades: pd.DataFrame, max_open_trades_per_asset: int) -> pd.DataFrame:
    if trades.empty:
        return trades
    if max_open_trades_per_asset < 1:
        raise ValueError("max_open_trades_per_asset must be >= 1")
...
def build_trades(df: pd.DataFrame, policy: PolicySpec, fee_bps: float, max_open_trades_per_asset: int) -> pd.DataFrame:
    ...
    trades["fee_fraction"] = fee_fraction
    trades["trade_return"] = trades["gross_trade_return"] - fee_fraction
...
def export_policy_outputs(trades: pd.DataFrame, output_dir: Path, policy: PolicySpec) -> tuple[Path, Path] | tuple[None, None]:
    if trades.empty:
        return None, None
    ...
    equity = trades[["entry_ts_utc", "policy_name", "equity_curve", "drawdown"]].copy()
    equity = equity.rename(columns={"entry_ts_utc": "ts_utc"})
