...
class PullbackReclaimAtrStrategy:
    name = "pullback_reclaim_atr_v2"

    def __init__(
        self,
        atr_pct_min: float = 1.0,
        asset_class: str = "MID_ALT",
        ema200_slope_lookback: int = 10,
    ) -> None:
        self.asset_class = str(asset_class or "MID_ALT").strip().upper()
        self.atr_pct_min = float(atr_pct_min)
        self.ema200_slope_lookback = int(ema200_slope_lookback)
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["ema200_slope"] = out["ema200"] - out["ema200"].shift(self.ema200_slope_lookback)
        ...
        out["atr_ok"] = out["atr_pct"] >= self.atr_pct_min
