...
def test_live_plan_fails_before_any_followup_or_client_construction(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(worker, "_fetch_actionable_plans", lambda: [_plan(execution_mode="LIVE")])
...
def test_paper_import_loads_only_database_environment(tmp_path: Path) -> None:
    sentinel_keys = (
        ...
        "SYNTH_DECISION_GATE_PERMISSION_PRIVATE_KEY",
        "SYNTH_DECISION_GATE_PERMISSION_PUBLIC_KEY",
    )
...
def test_worker_source_contains_no_private_or_broker_write_path() -> None:
    ...
    assert "execution_mode = 'PAPER'" in paper_runner
