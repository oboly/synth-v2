...
def test_snapshot_calls_balance_and_open_orders() -> None:
    """Snapshot must call both get_balance and get_open_orders exactly once each."""
    db = _next_db()
    seed = _seed_schema(db)
    kv, kb = parse_master_key(generate_test_master_key())
    ta_id = _seed_account_with_credential(seed, kb, kv)
...
if __name__ == "__main__":
    tests = [
        ...
        test_snapshot_calls_balance_and_open_orders,
