...
def test_artifact_json_is_byte_identical_across_runs() -> None:
    ...
    def _build() -> bytes:
        analysis = build_analysis(canonical=canonical, baseline_records=records["baseline"], enriched_records=records["enriched"], start=start, end=start + timedelta(days=1), venue="bitvavo")
